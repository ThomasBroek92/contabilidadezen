...
export function AdvogadosLeadForm() {
  ...
  const [formData, setFormData] = useState({
    nome: "", email: "", telefone: "", profissao: "", atividade: "", tributacao: "", faturamento: "", cidadeEstado: "", ehAdvogado: "", aceitaPolitica: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.ehAdvogado) infoAdicionais.push(`É advogado: ${formData.ehAdvogado}`);
      const { error } = await supabase.from('leads').insert({
        nome: result.data.nome, email: result.data.email, whatsapp: result.data.telefone,
        segmento: 'advogados', fonte: 'landing-page-advogados',
        cargo: formData.profissao || null,
        observacoes: infoAdicionais.length > 0 ? infoAdicionais.join(' | ') : null,
      });
      if (error) throw error;
      const whatsappUrl = getWhatsAppLink(WHATSAPP_MESSAGES.advogados);
      setSubmittedWhatsAppUrl(whatsappUrl);
      setIsSubmitted(true);
      toast({ title: "Formulário enviado com sucesso!", description: "Em breve um especialista entrará em contato." });
      resetHoneypot();
      setFormData({ nome: "", email: "", telefone: "", profissao: "", atividade: "", tributacao: "", faturamento: "", cidadeEstado: "", ehAdvogado: "", aceitaPolitica: false });
    } catch (error) {
  ...
  return (
    <section id="lead-form" className="py-16 lg:py-24 bg-[#F8FAFC]">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="bg-card rounded-2xl shadow-card p-8 lg:p-10 border border-[#334155]/20 order-2 lg:order-1">
            {isSubmitted ? (
            ...
            <p className="text-muted-foreground mb-8">Não deixe a alta carga tributária prejudicar o crescimento do seu escritório! Preencha o formulário para uma análise gratuita.</p>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="profissao" placeholder="Ex: Advogado, Sócio de Escritório" value={formData.profissao} onChange={(e) => setFormData({...formData, profissao: e.target.value})} />
                </div>
              </div>
              <div className="grid sm:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <Select value={formData.atividade} onValueChange={(value) => setFormData({...formData, atividade: value})}>
                    ...
                    <SelectContent>
                      <SelectItem value="escritorio-advocacia">Escritório de Advocacia</SelectItem>
                      <SelectItem value="advogado-autonomo">Advogado Autônomo</SelectItem>
                      <SelectItem value="sociedade-advogados">Sociedade de Advogados (OAB)</SelectItem>
                      <SelectItem value="advogado-pj">Advogado PJ</SelectItem>
                      <SelectItem value="consultoria-juridica">Consultoria Jurídica</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.ehAdvogado} onValueChange={(value) => setFormData({...formData, ehAdvogado: value})}>
                  <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="sim">Sim, sou advogado inscrito na OAB</SelectItem>
                    <SelectItem value="nao-gestor">Não, sou gestor/administrador do escritório</SelectItem>
                    <SelectItem value="estagiario">Sou estagiário/associado</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="flex items-start space-x-3 pt-2">
                ...
                <Label htmlFor="politica" className="text-sm text-muted-foreground leading-relaxed">
                  Li e concordo com a{" "}<a href="/politica-de-privacidade" className="text-secondary hover:underline">Política de Privacidade</a>
                </Label>
              </div>
              <input {...honeypotProps} />
              <Button type="submit" size="lg" className="w-full mt-4 bg-[#334155] hover:bg-[#1E293B] text-white" disabled={isSubmitting}>
          ...
          <div className="space-y-8 order-1 lg:order-2">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4">
                Controle a saúde financeira do seu escritório e{" "}
                <span className="text-[#334155]">pague menos impostos!</span>
              </h2>
              <p className="text-lg text-muted-foreground leading-relaxed">
                Seu escritório merece mais do que apenas acompanhamento. Assim como uma boa defesa jurídica, 
                suas finanças exigem estratégia e atenção constante.
              </p>
            </div>
            ...
            <div className="space-y-4">
              {["Planejamento tributário personalizado para advogados", "Redução legal de impostos comprovada", "Atendimento especializado no setor jurídico", "Suporte contábil dedicado para sua rotina"].map((item, index) => (
            ...
            <div className="p-6 bg-[#E2E8F0] rounded-xl border border-[#334155]/20">
              <p className="text-lg font-semibold text-foreground mb-4">Quanto você pode economizar?</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="p-4 bg-card rounded-lg border border-[#334155]/15">
                  <TrendingDown className="h-6 w-6 text-[#334155] mx-auto mb-2" />
                  <p className="text-2xl font-bold text-[#1E293B]">Fator R</p>
                  <p className="text-xs text-muted-foreground">Redução de alíquota</p>
                </div>
                <div className="p-4 bg-card rounded-lg border border-[#334155]/15">
                  <Percent className="h-6 w-6 text-[#334155] mx-auto mb-2" />
                  <p className="text-2xl font-bold text-[#1E293B]">6%</p>
