...
export function EcommerceLeadForm() {
  ...
  const [formData, setFormData] = useState({ nome: "", email: "", telefone: "", tipoNegocio: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const info: string[] = [];
      if (formData.tipoNegocio) info.push(`Tipo: ${formData.tipoNegocio}`);
      ...
      const { error } = await supabase.from('leads').insert({ nome: result.data.nome, email: result.data.email, whatsapp: result.data.telefone, segmento: 'ecommerce', fonte: 'landing-page-ecommerce', cargo: formData.tipoNegocio || null, observacoes: info.length > 0 ? info.join(' | ') : null });
      if (error) throw error;
      setSubmittedWhatsAppUrl(getWhatsAppLink(WHATSAPP_MESSAGES.ecommerce));
      setIsSubmitted(true);
      toast({ title: "Enviado!", description: "Em breve entraremos em contato." });
      resetHoneypot();
      setFormData({ nome: "", email: "", telefone: "", tipoNegocio: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
    } catch { toast({ title: "Erro", variant: "destructive" }); } finally { setIsSubmitting(false); }
  };

  return (
    <section id="lead-form" className="py-16 lg:py-24 bg-[#FDF2F8]">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="bg-card rounded-2xl shadow-card p-8 lg:p-10 border border-[#DB2777]/20 order-2 lg:order-1">
            {isSubmitted ? (
              ...
              <>
                <h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">Seu e-commerce com contabilidade certa</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2"><Label>Tipo de negócio</Label><Select value={formData.tipoNegocio} onValueChange={v => setFormData({...formData, tipoNegocio: v})}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent><SelectItem value="loja-propria">Loja própria</SelectItem><SelectItem value="marketplace">Marketplace</SelectItem><SelectItem value="dropshipping">Dropshipping</SelectItem><SelectItem value="infoproduto-fisico">Infoproduto + Físico</SelectItem></SelectContent></Select></div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  <input {...honeypotProps} />
                  <Button type="submit" size="lg" className="w-full mt-4 bg-[#DB2777] hover:bg-[#BE185D] text-white" disabled={isSubmitting}>{isSubmitting ? "Enviando..." : <><Send className="h-4 w-4 mr-2" />Enviar diagnóstico gratuito</>}</Button>
                </form>
              </>
            )}
          </div>
          <div className="space-y-8 order-1 lg:order-2">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4">Seu e-commerce com{" "}<span className="text-[#DB2777]">contabilidade especializada!</span></h2>
              <p className="text-lg text-muted-foreground leading-relaxed">Controle fiscal de estoque, CMV, ICMS-ST e emissão de notas fiscais para todas as plataformas.</p>
            </div>
            <div className="space-y-4">
              {["Controle de estoque e CMV automatizado", "Substituição tributária (ICMS-ST) correta", "Emissão de NF para marketplaces", "Dropshipping nacional e internacional"].map((item, i) => (
            ...
            <div className="p-6 bg-[#FCE7F3] rounded-xl border border-[#DB2777]/20">
              <p className="text-lg font-semibold text-foreground mb-4">Quanto você pode economizar?</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="p-4 bg-card rounded-lg border border-[#DB2777]/15"><TrendingDown className="h-6 w-6 text-[#DB2777] mx-auto mb-2" /><p className="text-2xl font-bold text-[#BE185D]">CMV</p><p className="text-xs text-muted-foreground">Otimizado</p></div>
                <div className="p-4 bg-card rounded-lg border border-[#DB2777]/15"><Percent className="h-6 w-6 text-[#DB2777] mx-auto mb-2" /><p className="text-2xl font-bold text-[#BE185D]">6%</p><p className="text-xs text-muted-foreground">A partir de</p></div>
