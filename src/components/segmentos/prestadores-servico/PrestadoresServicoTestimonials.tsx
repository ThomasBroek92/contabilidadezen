...
const fallbackTestimonials = [
  { name: "Carlos Silva", role: "Consultor de Gestão", location: "São Paulo, SP", content: "Estava pagando mais de 15% de impostos como consultor. Com o planejamento da Contabilidade Zen, migrei para o Anexo III e agora pago 6%. Economia real!", rating: 5 },
  { name: "Fernanda Lima", role: "Freelancer de Marketing", location: "Campinas, SP", content: "Finalmente tenho uma contabilidade que entende a realidade do freelancer. NFS-e automática, impostos em dia e zero dor de cabeça.", rating: 5 },
  { name: "Roberto Santos", role: "Consultor de TI", location: "Curitiba, PR", content: "Migrei de autônomo para PJ e a economia foi absurda. O time da Zen cuidou de tudo, desde a abertura da empresa até o planejamento tributário.", rating: 5 },
];
...
export function PrestadoresServicoTestimonials() {
  ...
  const renderStars = (rating: number) => [...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-[#D97706] text-[#D97706]' : 'text-muted-foreground/30'}`} />));
    ...
    <section className="py-16 lg:py-24 bg-[#FFFBEB]">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-sm font-semibold text-[#B45309] uppercase tracking-wider">Depoimentos</span>
          ...
          <p className="text-muted-foreground text-lg">{hasGMBReviews ? "Veja o que nossos clientes estão dizendo sobre a Contabilidade Zen no Google." : "Confira os resultados que nossos clientes prestadores de serviço estão tendo"}</p>
          {gmbStats && (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-3 mt-6 px-5 py-3 bg-card border border-border rounded-xl hover:border-[#D97706]/50 hover:shadow-card transition-all duration-300 group hover:-translate-y-0.5">
          ...
          <CarouselContent className="-ml-4">
            {displayFallback ? fallbackTestimonials.map((t, index) => (
              <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#D97706]/15 hover:border-[#D97706]/40 transition-all duration-300 h-full hover:-translate-y-1 hover:shadow-card relative">
                  <Quote className="absolute top-4 right-4 h-8 w-8 text-[#D97706]/20" /><div className="flex gap-1 mb-4">{renderStars(t.rating)}</div><p className="text-foreground/80 leading-relaxed mb-6 line-clamp-4">"{t.content}"</p>
                  <div className="flex items-center gap-4 pt-4 border-t border-border"><div className="w-12 h-12 rounded-full bg-[#FDE68A] flex items-center justify-center"><span className="text-lg font-bold text-[#B45309]">{t.name.split(" ").map(n => n[0]).join("")}</span></div><div><p className="font-semibold text-foreground">{t.name}</p><p className="text-sm text-muted-foreground">{t.role} • {t.location}</p></div></div>
                </div>
              </CarouselItem>
            )) : gmbReviews?.map((review) => (
              <CarouselItem key={review.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#D97706]/15 hover:border-[#D97706]/40 transition-all duration-300 relative h-full hover:-translate-y-1 hover:shadow-card">
                  ...
                  <Quote className="h-8 w-8 text-[#D97706]/30 mb-4" /><p className="text-foreground/80 leading-relaxed mb-6 line-clamp-4">"{review.comment || 'Excelente serviço!'}"</p><div className="flex gap-1 mb-4">{renderStars(review.rating)}</div>
                  <div className="flex items-center gap-3">{review.reviewer_photo_url ? (<img src={review.reviewer_photo_url} alt={review.reviewer_name} width={40} height={40} loading="lazy" decoding="async" className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" />) : (<div className="w-10 h-10 rounded-full bg-[#FDE68A] flex items-center justify-center"><span className="text-[#B45309] font-semibold text-sm">{review.reviewer_name.charAt(0).toUpperCase()}</span></div>)}<div><p className="font-semibold text-foreground">{review.reviewer_name}</p><p className="text-xs text-muted-foreground">{formatRelativeTime(review.review_time)}</p></div></div>
                </div>
              </CarouselItem>
            ))}
          </CarouselContent>
        ...
        <div className="text-center mt-10">
          {hasGMBReviews ? (<a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-2 text-[#B45309] hover:text-[#D97706] font-medium transition-colors">Ver todas as avaliações no Google<ExternalLink className="h-4 w-4" /></a>) : (<Button size="lg" variant="outline" className="border-secondary text-secondary hover:bg-secondary/10" onClick={scrollToForm}>Fale com um especialista</Button>)}
        </div>
      </div>
    </section>
  );
