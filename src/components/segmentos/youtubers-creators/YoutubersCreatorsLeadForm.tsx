...
export function YoutubersCreatorsLeadForm() {
  ...
  const [formData, setFormData] = useState({ nome: "", email: "", telefone: "", tipoConteudo: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const info: string[] = [];
      if (formData.tipoConteudo) info.push(`Tipo: ${formData.tipoConteudo}`);
      ...
      const { error } = await supabase.from('leads').insert({ nome: result.data.nome, email: result.data.email, whatsapp: result.data.telefone, segmento: 'youtubers-creators', fonte: 'landing-page-youtubers-creators', cargo: formData.tipoConteudo || null, observacoes: info.length > 0 ? info.join(' | ') : null });
      if (error) throw error;
      setSubmittedWhatsAppUrl(getWhatsAppLink(WHATSAPP_MESSAGES.youtubersCreators));
      setIsSubmitted(true);
      toast({ title: "Enviado!", description: "Em breve entraremos em contato." });
      resetHoneypot();
      setFormData({ nome: "", email: "", telefone: "", tipoConteudo: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
    } catch { toast({ title: "Erro", variant: "destructive" }); } finally { setIsSubmitting(false); }
  };

  return (
    <section id="lead-form" className="py-16 lg:py-24 bg-[#FEF2F2]">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="bg-card rounded-2xl shadow-card p-8 lg:p-10 border border-[#EF4444]/20 order-2 lg:order-1">
            {isSubmitted ? (
              ...
              <>
                <h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">Contabilidade certa para creators</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2"><Label>Tipo de conteúdo</Label><Select value={formData.tipoConteudo} onValueChange={v => setFormData({...formData, tipoConteudo: v})}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent><SelectItem value="youtube">YouTube</SelectItem><SelectItem value="streaming">Streaming (Twitch/Kick)</SelectItem><SelectItem value="instagram-tiktok">Instagram / TikTok</SelectItem><SelectItem value="podcast">Podcast</SelectItem></SelectContent></Select></div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div className="space-y-2"><Label>Faturamento mensal</Label><Select value={formData.faturamento} onValueChange={v => setFormData({...formData, faturamento: v})}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent><SelectItem value="ate-5k">Até R$ 5.000</SelectItem><SelectItem value="5k-15k">R$ 5.000 a R$ 15.000</SelectItem><SelectItem value="15k-50k">R$ 15.000 a R$ 50.000</SelectItem><SelectItem value="acima-50k">Acima de R$ 50.000</SelectItem></SelectContent></Select></div>
                  ...
                  <input {...honeypotProps} />
                  <Button type="submit" size="lg" className="w-full mt-4 bg-[#EF4444] hover:bg-[#DC2626] text-white" disabled={isSubmitting}>{isSubmitting ? "Enviando..." : <><Send className="h-4 w-4 mr-2" />Enviar diagnóstico gratuito</>}</Button>
                </form>
              </>
            )}
          </div>
          <div className="space-y-8 order-1 lg:order-2">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4">Seu canal com{" "}<span className="text-[#EF4444]">contabilidade especializada!</span></h2>
              <p className="text-lg text-muted-foreground leading-relaxed">AdSense, contratos com marcas, receitas de plataformas e direitos autorais — tudo no regime tributário correto.</p>
            </div>
            <div className="space-y-4">
              {["AdSense e monetização de plataformas", "Contratos com marcas e publis", "Direitos autorais e licenciamento", "Pessoa Física vs PJ: melhor enquadramento"].map((item, i) => (
            ...
            <div className="p-6 bg-[#FEE2E2] rounded-xl border border-[#EF4444]/20">
              <p className="text-lg font-semibold text-foreground mb-4">Quanto você pode economizar?</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="p-4 bg-card rounded-lg border border-[#EF4444]/15"><TrendingDown className="h-6 w-6 text-[#EF4444] mx-auto mb-2" /><p className="text-2xl font-bold text-[#DC2626]">27,5%</p><p className="text-xs text-muted-foreground">Pessoa Física</p></div>
                <div className="p-4 bg-card rounded-lg border border-[#EF4444]/15"><Percent className="h-6 w-6 text-[#EF4444] mx-auto mb-2" /><p className="text-2xl font-bold text-[#DC2626]">6%</p><p className="text-xs text-muted-foreground">Como PJ</p></div>
