...
import { StaggerContainer, StaggerItem, HoverLift, AnimatedIcon } from "@/components/ui/scroll-animation";
  ...
  return (
    <section className="py-20 lg:py-28 bg-muted/30">
      <div className="container mx-auto px-4">
        ...
        <StaggerItem type="scale" className="mb-10">
          <HoverLift lift={6}>
            <div className="relative bg-gradient-to-br from-secondary via-secondary/90 to-secondary/80 rounded-3xl p-8 lg:p-12 overflow-hidden">
              ...
              <div className="relative grid lg:grid-cols-2 gap-8 items-center">
                ...
                <StaggerContainer>
                  ...
                  <StaggerItem type="slide">
                    <ul className="grid sm:grid-cols-2 gap-3 mb-8">
                      {[
                        ...
                        <li key={idx} className="flex items-center gap-2 text-white/90 group">
                          <CheckCircle className="h-5 w-5 text-white flex-shrink-0 group-hover:scale-110 transition-transform" />
                          <span>{benefit}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <StaggerItem type="scale">
                    <div className="flex flex-wrap items-center gap-4">
                      <Button 
                        size="lg" 
                        className="bg-white text-secondary hover:bg-white/90 font-semibold hover:scale-105 transition-transform"
                        asChild
                      >
                ...
                <StaggerItem type="scale" className="lg:block space-y-4">
        ...
        <StaggerItem type="hybrid" className="mb-10">
          <HoverLift lift={4}>
              ...
              <div className="relative flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div className="hidden xl:flex items-center gap-4 shrink-0">
                  {["100% Digital", "15 dias", "Sem interrupção"].map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-lg hover:bg-white/20 transition-colors">
                ...
                <Button 
                  size="lg"
                  className="bg-white text-primary hover:bg-white/90 font-semibold shrink-0 hover:scale-105 transition-transform"
                  asChild
                >
                  <Link to="/contato">
                    Migrar Agora
                ...
                {otherServices.map((service, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <HoverLift lift={6} className="h-full">
                      <div className="h-full bg-card rounded-2xl p-6 border border-border hover:border-secondary/50 transition-all duration-300 flex flex-col">
                        {/* Icon & Title */}
                        ...
                        <ul className="space-y-2 mb-6">
                          {service.benefits.map((benefit, idx) => (
                            <li key={idx} className="flex items-center gap-2 text-sm group">
                              <CheckCircle className="h-4 w-4 text-secondary flex-shrink-0 group-hover:scale-110 transition-transform" />
