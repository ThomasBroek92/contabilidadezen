...
// Fallback testimonials específicos para representantes
...
export function RepresentantesTestimonials() {
  ...
  const renderStars = (rating: number) =>
    [...Array(5)].map((_, i) => (
      <Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-[#E87C1E] text-[#E87C1E]' : 'text-muted-foreground/30'}`} />
    ));
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          {gmbStats && (
            ...
            >
              ...
              <ExternalLink className="h-4 w-4 text-muted-foreground group-hover:text-[#E87C1E] transition-colors" />
            </a>
          )}
        </div>
            ...
            {displayFallback
              ? fallbackTestimonials.map((t, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#E87C1E]/15 hover:border-[#E87C1E]/40 transition-all duration-300 h-full hover:-translate-y-1 hover:shadow-card relative">
                      ...
                      <div className="flex items-center gap-4 pt-4 border-t border-border">
                        <div className="w-12 h-12 rounded-full bg-[#FDE8CC] flex items-center justify-center">
                          <span className="text-lg font-bold text-[#C4680F]">{t.name.split(" ").map(n => n[0]).join("")}</span>
                        </div>
                        <div>
              ...
              : gmbReviews?.map((review) => (
                  <CarouselItem key={review.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#E87C1E]/15 hover:border-[#E87C1E]/40 transition-all duration-300 relative h-full hover:-translate-y-1 hover:shadow-card">
                      ...
                      <Quote className="h-8 w-8 text-[#E87C1E]/30 mb-4" />
                      ...
                      <div className="flex items-center gap-3">
                        {review.reviewer_photo_url ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-[#FDE8CC] flex items-center justify-center">
                            <span className="text-[#C4680F] font-semibold text-sm">{review.reviewer_name.charAt(0).toUpperCase()}</span>
                          </div>
                        )}
                        <div>
