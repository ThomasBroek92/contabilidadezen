...
function StepCard({ step, index }: { step: ProcessStep; index: number }) {
  ...
  return (
    <div className={`relative h-full rounded-2xl p-6 md:p-8 transition-all duration-300 border-2 group hover:scale-[1.02] hover:shadow-xl ${isClient ? "bg-gradient-to-br from-[#D1FAE5] to-[#A7F3D0] border-[#10B981]/20 hover:border-[#10B981]/40" : "bg-gradient-to-br from-secondary/5 to-accent/5 border-secondary/20 hover:border-secondary/40"}`} style={{ animationDelay: shouldReduceMotion ? "0ms" : `${index * 100}ms` }}>
      <div className="flex items-center justify-between mb-6">
        <span className={`text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full ${isClient ? "bg-[#10B981]/10 text-[#059669]" : "bg-secondary/10 text-secondary"}`}>{isClient ? "Sua Parte" : "Contabilidade Zen"}</span>
        <span className={`text-3xl md:text-4xl font-black ${isClient ? "text-[#10B981]/20" : "text-secondary/20"}`}>{step.number}</span>
      </div>
      <div className={`w-14 h-14 md:w-16 md:h-16 rounded-xl flex items-center justify-center mb-5 transition-transform duration-300 group-hover:scale-110 ${isClient ? "bg-gradient-to-br from-[#10B981] to-[#059669]" : "bg-gradient-to-br from-secondary to-accent"}`}>
        <Icon className="h-7 w-7 md:h-8 md:w-8 text-white" />
      </div>
      <h3 className={`font-bold text-lg md:text-xl mb-3 ${isClient ? "text-[#065F46]" : "text-foreground"}`}>{step.title}</h3>
      <p className="text-muted-foreground text-sm md:text-base leading-relaxed">{step.description}</p>
      <div className={`absolute bottom-0 right-0 w-24 h-24 rounded-tl-[4rem] opacity-10 ${isClient ? "bg-gradient-to-br from-[#10B981] to-[#059669]" : "bg-gradient-to-br from-secondary to-accent"}`} />
    </div>
  );
}
    ...
    <section className="py-16 md:py-24 bg-[#D1FAE5]">
      <div className="container mx-auto px-4">
        <div className="text-center mb-6">
          <span className="inline-block text-[#059669] font-semibold text-sm uppercase tracking-wider mb-3">Processo Simplificado</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">Dê o próximo passo para{" "}<span className="text-[#10B981]">pagar menos impostos</span></h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Você cuida apenas de{" "}<strong className="text-[#059669]">{clientCount} etapas simples</strong>, nós cuidamos de{" "}<strong className="text-secondary">{zenCount} processos complexos</strong></p>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8 mb-10" style={{ animationDelay: shouldReduceMotion ? "0ms" : "100ms" }}>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded-full bg-gradient-to-br from-[#10B981] to-[#059669]" /><span className="text-sm font-medium text-muted-foreground">Sua responsabilidade</span></div>
        ...
        <div className="flex items-center justify-center gap-0 mt-8">
          {Array.from({ length: count }).map((_, index) => (<button key={index} onClick={() => api?.scrollTo(index)} className="p-[7px] cursor-pointer" aria-label={`Ir para slide ${index + 1}`}><span className={`block h-2.5 rounded-full transition-all duration-300 ${current === index ? "bg-[#10B981] w-8" : "w-2.5 bg-muted-foreground/30 hover:bg-muted-foreground/50"}`} /></button>))}
        </div>
        <div className="text-center mt-10">
          <Button size="lg" className="bg-[#10B981] hover:bg-[#059669] text-white font-semibold gap-2" onClick={scrollToForm}>Comece agora e reduza impostos<ArrowRight className="h-4 w-4" /></Button>
        </div>
      </div>
    </section>
  );
