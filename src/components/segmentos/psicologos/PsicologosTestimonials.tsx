...
const fallbackTestimonials = [
  { name: "Dra. Camila Rodrigues", role: "Psicóloga Clínica", location: "São Paulo, SP", content: "Desde que contratei a Contabilidade Zen, consegui reduzir mais de 28% dos meus impostos. Finalmente tenho tranquilidade para focar nos meus pacientes.", rating: 5 },
  { name: "Dr. Ricardo Santos", role: "Proprietário de Clínica", location: "Rio de Janeiro, RJ", content: "A equipe é extremamente atenciosa e entende perfeitamente as necessidades da área de psicologia. Recomendo para todos os colegas de profissão!", rating: 5 },
  { name: "Dra. Patricia Mendes", role: "Neuropsicóloga", location: "Brasília, DF", content: "Migrei de Pessoa Física para PJ com a orientação deles e a economia foi impressionante. Atendimento impecável do início ao fim.", rating: 5 },
];
...
export function PsicologosTestimonials() {
  ...
  const renderStars = (rating: number) => [...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-[#8B5CF6] text-[#8B5CF6]' : 'text-muted-foreground/30'}`} />));
    ...
    <section className="py-16 lg:py-24 bg-[#F5F3FF]">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-sm font-semibold text-[#7C3AED] uppercase tracking-wider">Depoimentos</span>
          ...
          <p className="text-muted-foreground text-lg">{hasGMBReviews ? "Veja o que nossos clientes estão dizendo sobre a Contabilidade Zen no Google." : "Confira os resultados que nossos clientes psicólogos estão tendo com nosso apoio"}</p>
          {gmbStats && (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-3 mt-6 px-5 py-3 bg-card border border-border rounded-xl hover:border-[#8B5CF6]/50 hover:shadow-card transition-all duration-300 group hover:-translate-y-0.5">
              ...
              <ExternalLink className="h-4 w-4 text-muted-foreground group-hover:text-[#8B5CF6] transition-colors" />
            </a>
          )}
        </div>
            ...
            {displayFallback
              ? fallbackTestimonials.map((t, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#8B5CF6]/15 hover:border-[#8B5CF6]/40 transition-all duration-300 h-full hover:-translate-y-1 hover:shadow-card relative">
                      <Quote className="absolute top-4 right-4 h-8 w-8 text-[#8B5CF6]/20" />
                      ...
                      <div className="flex items-center gap-4 pt-4 border-t border-border">
                        <div className="w-12 h-12 rounded-full bg-[#DDD6FE] flex items-center justify-center"><span className="text-lg font-bold text-[#7C3AED]">{t.name.split(" ").map(n => n[0]).join("")}</span></div>
              ...
              : gmbReviews?.map((review) => (
                  <CarouselItem key={review.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#8B5CF6]/15 hover:border-[#8B5CF6]/40 transition-all duration-300 relative h-full hover:-translate-y-1 hover:shadow-card">
                      ...
                      <div className="flex items-center gap-3">
                        {review.reviewer_photo_url ? (<img src={review.reviewer_photo_url} alt={review.reviewer_name} width={40} height={40} loading="lazy" decoding="async" className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" />) : (<div className="w-10 h-10 rounded-full bg-[#DDD6FE] flex items-center justify-center"><span className="text-[#7C3AED] font-semibold text-sm">{review.reviewer_name.charAt(0).toUpperCase()}</span></div>)}
        ...
        <div className="text-center mt-10">
          {hasGMBReviews ? (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-2 text-[#7C3AED] hover:text-[#8B5CF6] font-medium transition-colors">Ver todas as avaliações no Google<ExternalLink className="h-4 w-4" /></a>
