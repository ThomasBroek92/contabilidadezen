...
async function requestIndexing(accessToken: string, url: string, type: string = 'URL_UPDATED'): Promise<{ success: boolean; result?: any; error?: string }> {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch pending items from queue (max 100 per batch due to Google quota)
