...
const fallbackTestimonials = [
  { name: "Pedro Henrique", role: "YouTuber", location: "São Paulo, SP", content: "Estava pagando 27,5% de IR no AdSense. A Contabilidade Zen abriu meu CNPJ e agora pago apenas 6%. Economia absurda!", rating: 5 },
  { name: "Marina Costa", role: "Influenciadora Digital", location: "Rio de Janeiro, RJ", content: "Finalmente consigo emitir nota fiscal para as marcas. Meus contratos de publi triplicaram depois que abri o CNPJ com a Zen.", rating: 5 },
  { name: "Gabriel Lima", role: "Streamer Twitch", location: "Curitiba, PR", content: "Recebia da Twitch como PF e tinha medo da Receita Federal. A equipe da Zen regularizou tudo e cuida dos meus impostos todo mês.", rating: 5 },
];
...
export function YoutubersCreatorsTestimonials() {
  ...
  const renderStars = (rating: number) => [...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-[#EF4444] text-[#EF4444]' : 'text-muted-foreground/30'}`} />));
    ...
    <section className="py-16 lg:py-24 bg-[#FEF2F2]">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <span className="text-sm font-semibold text-[#DC2626] uppercase tracking-wider">Depoimentos</span>
          ...
          <p className="text-muted-foreground text-lg">{hasGMBReviews ? "Veja o que nossos clientes estão dizendo no Google." : "Confira os resultados que nossos clientes creators estão tendo"}</p>
          {gmbStats && (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-3 mt-6 px-5 py-3 bg-card border border-border rounded-xl hover:border-[#EF4444]/50 hover:shadow-card transition-all duration-300 group hover:-translate-y-0.5">
          ...
          <CarouselContent className="-ml-4">
            {displayFallback ? fallbackTestimonials.map((t, index) => (
              <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#EF4444]/15 hover:border-[#EF4444]/40 transition-all duration-300 h-full hover:-translate-y-1 hover:shadow-card relative">
                  <Quote className="absolute top-4 right-4 h-8 w-8 text-[#EF4444]/20" /><div className="flex gap-1 mb-4">{renderStars(t.rating)}</div><p className="text-foreground/80 leading-relaxed mb-6 line-clamp-4">"{t.content}"</p>
                  <div className="flex items-center gap-4 pt-4 border-t border-border"><div className="w-12 h-12 rounded-full bg-[#FECACA] flex items-center justify-center"><span className="text-lg font-bold text-[#DC2626]">{t.name.split(" ").map(n => n[0]).join("")}</span></div><div><p className="font-semibold text-foreground">{t.name}</p><p className="text-sm text-muted-foreground">{t.role} • {t.location}</p></div></div>
                </div>
              </CarouselItem>
            )) : gmbReviews?.map((review) => (
              <CarouselItem key={review.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#EF4444]/15 hover:border-[#EF4444]/40 transition-all duration-300 relative h-full hover:-translate-y-1 hover:shadow-card">
                  ...
                  <Quote className="h-8 w-8 text-[#EF4444]/30 mb-4" /><p className="text-foreground/80 leading-relaxed mb-6 line-clamp-4">"{review.comment || 'Excelente serviço!'}"</p><div className="flex gap-1 mb-4">{renderStars(review.rating)}</div>
                  <div className="flex items-center gap-3">{review.reviewer_photo_url ? (<img src={review.reviewer_photo_url} alt={review.reviewer_name} width={40} height={40} loading="lazy" decoding="async" className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" />) : (<div className="w-10 h-10 rounded-full bg-[#FECACA] flex items-center justify-center"><span className="text-[#DC2626] font-semibold text-sm">{review.reviewer_name.charAt(0).toUpperCase()}</span></div>)}<div><p className="font-semibold text-foreground">{review.reviewer_name}</p><p className="text-xs text-muted-foreground">{formatRelativeTime(review.review_time)}</p></div></div>
                </div>
              </CarouselItem>
            ))}
          </CarouselContent>
        ...
        <div className="text-center mt-10">
          {hasGMBReviews ? (<a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-2 text-[#DC2626] hover:text-[#EF4444] font-medium transition-colors">Ver todas as avaliações no Google<ExternalLink className="h-4 w-4" /></a>) : (<Button size="lg" variant="outline" className="border-secondary text-secondary hover:bg-secondary/10" onClick={scrollToForm}>Fale com um especialista</Button>)}
        </div>
      </div>
    </section>
  );
