import { useState, useEffect } from 'react';
import { CategoryOption, COLUMN_COLORS } from '@/hooks/use-category-settings';
import {
  Dialog,
  DialogContent,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Loader2, Plus, Trash2, GripVertical, Save, Check } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
  PointerSensor,
  useSensor,
  useSensors,
  DragEndEvent,
} from '@dnd-kit/core';
import {
  arrayMove,
  SortableContext,
  sortableKeyboardCoordinates,
  useSortable,
  verticalListSortingStrategy,
} from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from '@/components/ui/popover';

interface CategorySettingsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  categories: CategoryOption[];
  onSave: (categories: CategoryOption[]) => void;
  isSaving: boolean;
}

interface SortableCategoryItemProps {
  category: CategoryOption;
  onUpdate: (id: string, updates: Partial<CategoryOption>) => void;
  onDelete: (id: string) => void;
  canDelete: boolean;
}

function getColorStyle(colorId: string) {
  return COLUMN_COLORS.find(c => c.id === colorId) || COLUMN_COLORS[0];
}

function SortableCategoryItem({ category, onUpdate, onDelete, canDelete }: SortableCategoryItemProps) {
  const [colorPickerOpen, setColorPickerOpen] = useState(false);
  const colorStyle = getColorStyle(category.color);
  
  const {
    attributes,
    listeners,
    setNodeRef,
    transform,
    transition,
    isDragging,
  } = useSortable({ id: category.id });

  const style = {
    transform: CSS.Transform.toString(transform),
    transition,
    opacity: isDragging ? 0.5 : 1,
  };

  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className="flex items-center gap-2 p-2 bg-white dark:bg-[#252526] border border-[#E9E9E7] dark:border-[#3F3F3F] rounded-sm"
    >
      <button
        {...attributes}
        {...listeners}
        className="cursor-grab active:cursor-grabbing p-1 hover:bg-[#F7F7F5] dark:hover:bg-[#3F3F3F] rounded-sm"
      >
        <GripVertical className="h-4 w-4 text-[#9B9A97]" />
      </button>

      <Popover open={colorPickerOpen} onOpenChange={setColorPickerOpen}>
        <PopoverTrigger asChild>
          <button
            className="w-6 h-6 rounded-sm border border-[#E9E9E7] dark:border-[#3F3F3F] flex items-center justify-center hover:scale-110 transition-transform"
            style={{ backgroundColor: colorStyle.bg }}
          />
        </PopoverTrigger>
        <PopoverContent className="w-auto p-2 bg-white dark:bg-[#252526] border-[#E9E9E7] dark:border-[#3F3F3F]" align="start">
          <div className="grid grid-cols-5 gap-1.5">
            {COLUMN_COLORS.map(color => (
              <button
                key={color.id}
                onClick={() => {
                  onUpdate(category.id, { color: color.id });
                  setColorPickerOpen(false);
                }}
                className="w-7 h-7 rounded-sm border border-[#E9E9E7] dark:border-[#3F3F3F] flex items-center justify-center hover:scale-110 transition-transform"
                style={{ backgroundColor: color.bg }}
                title={color.label}
              >
                {category.color === color.id && (
                  <Check className="h-3.5 w-3.5" style={{ color: color.text }} />
                )}
              </button>
            ))}
          </div>
        </PopoverContent>
      </Popover>

      <Input
        value={category.title}
        onChange={(e) => onUpdate(category.id, { title: e.target.value })}
        className="flex-1 h-8 text-sm bg-transparent border-[#E9E9E7] dark:border-[#3F3F3F] text-[#37352F] dark:text-[#FFFFFFCF]"
        placeholder="Nome do setor"
      />

      <button
        onClick={() => onDelete(category.id)}
        disabled={!canDelete}
        className={`p-1.5 rounded-sm transition-colors ${
          canDelete 
            ? 'hover:bg-[#FFE2DD] text-[#9B9A97] hover:text-[#E03E3E]' 
            : 'text-[#D3D3D3] cursor-not-allowed'
        }`}
      >
        <Trash2 className="h-4 w-4" />
      </button>
    </div>
  );
}

export function CategorySettingsDialog({ open, onOpenChange, categories, onSave, isSaving }: CategorySettingsDialogProps) {
  const [localCategories, setLocalCategories] = useState<CategoryOption[]>(categories);

  useEffect(() => {
    if (open) {
      setLocalCategories(categories);
    }
  }, [open, categories]);

  const sensors = useSensors(
    useSensor(PointerSensor),
    useSensor(KeyboardSensor, {
      coordinateGetter: sortableKeyboardCoordinates,
    })
  );

  const handleDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;

    if (over && active.id !== over.id) {
      setLocalCategories((items) => {
        const oldIndex = items.findIndex(i => i.id === active.id);
        const newIndex = items.findIndex(i => i.id === over.id);
        const newItems = arrayMove(items, oldIndex, newIndex);
        return newItems.map((item, index) => ({ ...item, order: index }));
      });
    }
  };

  const handleUpdate = (id: string, updates: Partial<CategoryOption>) => {
    setLocalCategories(cats => cats.map(c => c.id === id ? { ...c, ...updates } : c));
  };

  const handleDelete = (id: string) => {
    if (localCategories.length <= 1) return;
    setLocalCategories(cats => cats.filter(c => c.id !== id).map((c, i) => ({ ...c, order: i })));
  };

  const handleAdd = () => {
    const newId = `setor_${Date.now()}`;
    setLocalCategories(cats => [...cats, {
      id: newId,
      title: 'Novo Setor',
      color: 'gray',
      order: cats.length,
    }]);
  };

  const handleSave = () => {
    if (localCategories.some(c => !c.title.trim())) {
      return;
    }
    onSave(localCategories);
    onOpenChange(false);
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px] bg-white dark:bg-[#191919] border-[#E9E9E7] dark:border-[#2F2F2F] shadow-xl p-0">
        <DialogHeader className="px-6 pt-5 pb-3">
          <DialogTitle className="text-base font-semibold text-[#37352F] dark:text-[#FFFFFFCF]">
            Personalizar Setores
          </DialogTitle>
        </DialogHeader>

        <div className="px-6 py-2">
          <p className="text-xs text-[#9B9A97] mb-4">
            Arraste para reordenar, clique na cor para alterar, ou adicione novos setores.
          </p>

          <DndContext
            sensors={sensors}
            collisionDetection={closestCenter}
            onDragEnd={handleDragEnd}
          >
            <SortableContext
              items={localCategories.map(c => c.id)}
              strategy={verticalListSortingStrategy}
            >
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {localCategories.map(category => (
                  <SortableCategoryItem
                    key={category.id}
                    category={category}
                    onUpdate={handleUpdate}
                    onDelete={handleDelete}
                    canDelete={localCategories.length > 1}
                  />
                ))}
              </div>
            </SortableContext>
          </DndContext>

          <button
            onClick={handleAdd}
            className="w-full flex items-center justify-center gap-2 mt-3 py-2 text-sm text-[#9B9A97] hover:bg-[#F7F7F5] dark:hover:bg-[#252525] rounded-sm border border-dashed border-[#E9E9E7] dark:border-[#3F3F3F] transition-colors"
          >
            <Plus className="h-4 w-4" />
            Adicionar Setor
          </button>
        </div>

        <DialogFooter className="px-6 py-4 border-t border-[#E9E9E7] dark:border-[#2F2F2F] bg-[#FBFBFA] dark:bg-[#1E1E1E]">
          <Button 
            type="button" 
            variant="ghost" 
            onClick={() => onOpenChange(false)}
            className="text-[#37352F] dark:text-[#FFFFFFCF] hover:bg-[#E9E9E7] dark:hover:bg-[#3F3F3F] shadow-none"
          >
            Cancelar
          </Button>
          <Button 
            onClick={handleSave}
            disabled={isSaving || localCategories.some(c => !c.title.trim())}
            className="bg-[#2383E2] hover:bg-[#1B6EC2] text-white shadow-none"
          >
            {isSaving ? (
              <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            ) : (
              <Save className="h-4 w-4 mr-2" />
            )}
            Salvar Setores
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
