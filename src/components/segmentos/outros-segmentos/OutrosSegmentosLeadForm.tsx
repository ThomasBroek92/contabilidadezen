...
export function OutrosSegmentosLeadForm() {
  ...
  const [formData, setFormData] = useState({ nome: "", email: "", telefone: "", profissao: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('leads').insert({ nome: result.data.nome, email: result.data.email, whatsapp: result.data.telefone, segmento: 'outros-segmentos', fonte: 'landing-page-outros-segmentos', cargo: formData.profissao || null, observacoes: info.length > 0 ? info.join(' | ') : null });
      if (error) throw error;
      setSubmittedWhatsAppUrl(getWhatsAppLink(WHATSAPP_MESSAGES.outrosSegmentos));
      setIsSubmitted(true);
      toast({ title: "Enviado!", description: "Em breve entraremos em contato." });
      resetHoneypot();
      setFormData({ nome: "", email: "", telefone: "", profissao: "", faturamento: "", cidadeEstado: "", aceitaPolitica: false });
    ...
    <section id="lead-form" className="py-16 lg:py-24 bg-[#F8FAFC]">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="bg-card rounded-2xl shadow-card p-8 lg:p-10 border border-[#475569]/20 order-2 lg:order-1">
            {isSubmitted ? (
              ...
              <>
                <h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">Contabilidade para sua profissão</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2"><Label>Profissão</Label><Select value={formData.profissao} onValueChange={v => setFormData({...formData, profissao: v})}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent><SelectItem value="arquiteto-engenheiro">Arquiteto / Engenheiro</SelectItem><SelectItem value="designer-publicitario">Designer / Publicitário</SelectItem><SelectItem value="coach-mentor">Coach / Mentor</SelectItem><SelectItem value="fotografo-videomaker">Fotógrafo / Videomaker</SelectItem></SelectContent></Select></div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div className="space-y-2"><Label>Faturamento mensal</Label><Select value={formData.faturamento} onValueChange={v => setFormData({...formData, faturamento: v})}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent><SelectItem value="ate-10k">Até R$ 10.000</SelectItem><SelectItem value="10k-20k">R$ 10.000 a R$ 20.000</SelectItem><SelectItem value="20k-50k">R$ 20.000 a R$ 50.000</SelectItem><SelectItem value="acima-50k">Acima de R$ 50.000</SelectItem></SelectContent></Select></div>
                  ...
                  <input {...honeypotProps} />
                  <Button type="submit" size="lg" className="w-full mt-4 bg-[#475569] hover:bg-[#334155] text-white" disabled={isSubmitting}>{isSubmitting ? "Enviando..." : <><Send className="h-4 w-4 mr-2" />Enviar diagnóstico gratuito</>}</Button>
                </form>
              </>
            )}
          </div>
          <div className="space-y-8 order-1 lg:order-2">
            <div>
              <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4">Sua profissão com{" "}<span className="text-[#475569]">contabilidade especializada!</span></h2>
              <p className="text-lg text-muted-foreground leading-relaxed">Cada profissão tem regras tributárias específicas. Nosso planejamento tributário é personalizado para maximizar sua economia.</p>
            </div>
            <div className="space-y-4">
              {["Enquadramento tributário otimizado", "Fator R e pró-labore estratégico", "Abertura de CNPJ em até 7 dias", "Atendimento 100% digital"].map((item, i) => (
            ...
            <div className="p-6 bg-[#F1F5F9] rounded-xl border border-[#475569]/20">
              <p className="text-lg font-semibold text-foreground mb-4">Quanto você pode economizar?</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="p-4 bg-card rounded-lg border border-[#475569]/15"><TrendingDown className="h-6 w-6 text-[#475569] mx-auto mb-2" /><p className="text-2xl font-bold text-[#334155]">27,5%</p><p className="text-xs text-muted-foreground">Pessoa Física</p></div>
                <div className="p-4 bg-card rounded-lg border border-[#475569]/15"><Percent className="h-6 w-6 text-[#475569] mx-auto mb-2" /><p className="text-2xl font-bold text-[#334155]">6%</p><p className="text-xs text-muted-foreground">Como PJ</p></div>
