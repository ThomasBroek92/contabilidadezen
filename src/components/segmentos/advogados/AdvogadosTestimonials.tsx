...
const fallbackTestimonials = [
  { name: "Dr. Ricardo Mendes", role: "Advogado Tributarista", location: "São Paulo, SP", content: "Desde que contratei a Contabilidade Zen, consegui reduzir mais de 30% dos meus impostos. Finalmente tenho tranquilidade para focar nos meus clientes.", rating: 5 },
  { name: "Dra. Camila Santos", role: "Sócia de Escritório", location: "Campinas, SP", content: "A equipe é extremamente atenciosa e entende perfeitamente as necessidades do setor jurídico. A sociedade de advogados ficou muito mais organizada.", rating: 5 },
  { name: "Dr. André Oliveira", role: "Advogado Autônomo", location: "Curitiba, PR", content: "Migrei de Pessoa Física para PJ com a orientação deles e a economia foi impressionante. Atendimento impecável do início ao fim.", rating: 5 },
];
...
export function AdvogadosTestimonials() {
  ...
  const renderStars = (rating: number) => [...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < rating ? 'fill-[#334155] text-[#334155]' : 'text-muted-foreground/30'}`} />));
    ...
    <section className="py-16 lg:py-24 bg-[#F8FAFC]">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-muted-foreground text-lg">{hasGMBReviews ? "Veja o que nossos clientes estão dizendo sobre a Contabilidade Zen no Google." : "Confira os resultados que nossos clientes advogados estão tendo com nosso apoio"}</p>
          {gmbStats && (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-3 mt-6 px-5 py-3 bg-card border border-border rounded-xl hover:border-[#334155]/50 hover:shadow-card transition-all duration-300 group hover:-translate-y-0.5">
              ...
              <ExternalLink className="h-4 w-4 text-muted-foreground group-hover:text-[#334155] transition-colors" />
            </a>
          )}
        </div>
            ...
            {displayFallback
              ? fallbackTestimonials.map((t, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#334155]/15 hover:border-[#334155]/40 transition-all duration-300 h-full hover:-translate-y-1 hover:shadow-card relative">
                      <Quote className="absolute top-4 right-4 h-8 w-8 text-[#334155]/20" />
                      ...
                      <div className="flex items-center gap-4 pt-4 border-t border-border">
                        <div className="w-12 h-12 rounded-full bg-[#CBD5E1] flex items-center justify-center"><span className="text-lg font-bold text-[#334155]">{t.name.split(" ").map(n => n[0]).join("")}</span></div>
              ...
              : gmbReviews?.map((review) => (
                  <CarouselItem key={review.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    <div className="bg-card rounded-2xl p-6 lg:p-8 border border-[#334155]/15 hover:border-[#334155]/40 transition-all duration-300 relative h-full hover:-translate-y-1 hover:shadow-card">
                      ...
                      <Quote className="h-8 w-8 text-[#334155]/30 mb-4" />
                      ...
                      <div className="flex items-center gap-3">
                        {review.reviewer_photo_url ? (<img src={review.reviewer_photo_url} alt={review.reviewer_name} width={40} height={40} loading="lazy" decoding="async" className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" />) : (<div className="w-10 h-10 rounded-full bg-[#CBD5E1] flex items-center justify-center"><span className="text-[#334155] font-semibold text-sm">{review.reviewer_name.charAt(0).toUpperCase()}</span></div>)}
        ...
        <div className="text-center mt-10">
          {hasGMBReviews ? (
            <a href="https://g.page/r/CSe4RMezF61hEAI/review" target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center gap-2 text-[#334155] hover:text-[#1E293B] font-medium transition-colors">Ver todas as avaliações no Google<ExternalLink className="h-4 w-4" /></a>
